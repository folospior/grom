-module(stratus@internal@tcp).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "src/stratus/internal/tcp.gleam").
-export([connect/4, shutdown/2, send/2, 'receive'/2, receive_timeout/3, set_opts/2, controlling_process/2]).
...
-file("src/stratus/internal/tcp.gleam", 9).
?DOC(false).
-spec connect(
...
-file("src/stratus/internal/tcp.gleam", 17).
?DOC(false).
-spec shutdown(
...
-file("src/stratus/internal/tcp.gleam", 20).
?DOC(false).
-spec send(stratus@internal@socket:socket(), gleam@bytes_tree:bytes_tree()) -> {ok,
        nil} |
    {error, stratus@internal@socket:socket_reason()}.
send(Socket, Packet) ->
    stratus_ffi:tcp_send(Socket, Packet).

-file("src/stratus/internal/tcp.gleam", 23).
?DOC(false).
...
-file("src/stratus/internal/tcp.gleam", 26).
?DOC(false).
...
-file("src/stratus/internal/tcp.gleam", 33).
?DOC(false).
-spec set_opts(
    stratus@internal@socket:socket(),
    list({gleam@erlang@atom:atom_(), gleam@dynamic:dynamic_()})
...
-file("src/stratus/internal/tcp.gleam", 39).
?DOC(false).
...
controlling_process(Socket, New_owner) ->
    stratus_ffi:tcp_controlling_process(Socket, New_owner).
