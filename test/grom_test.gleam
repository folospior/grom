...
/// All use the `create_category_channel_response` function, although it shouldn't be much of an issue to use other functions,
...
pub fn error_response_1_test() {
  let assert Error(grom.ReceivedErrorResponse(_)) =
    echo grom.create_category_channel_response(
      response.new(200)
      |> response.set_body(
